...
type: pattern
summary: A pattern for designing and cultivating intentional partnerships within a
  broader ecosystem to create shared value and enhance collective resilience.
collection: commons-blueprint
aliases_list:
...
```mermaid
graph TD
    A[Shared Purpose] --> B{Ecosystem Partnership};
    C[Complementary Capabilities] --> B;
    D[Equitable Value Distribution] --> B;
    B --> E[Increased Innovation];
    B --> F[Enhanced Resilience];
    B --> G[Access to New Markets];
```
